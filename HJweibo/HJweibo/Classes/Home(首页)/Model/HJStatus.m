...
-(CGFloat)cellHeight
{
    ...
    HJCalculate *cal = [[HJCalculate alloc] init];
    
    //用户头像高度40 + 头像顶部的margin10
    cal.add(HJMargin).add(40);
    
    //微博文本内容的高度 + margin10
    NSString *weiboContent = [[HJTools shareManager] deleteHttpStr:_text];
    cal.add(HJMargin).add([self calculateTextHeight:weiboContent fontSize:WeiboContentFontSize]);
    ...
    if(_pic_urls.count == 0){
        ...
        if(_page_type == PageTypeAudio || _page_type == PageTypeWeb || _page_type == PageTypeText){
            
            self.webPageFrame = CGRectMake(HJMargin, cal.sum, PageWidth, WebPageHeight);
            
            cal.add(WebPageHeight).add(HJMargin);
            
            //获取短连接信息
            if([self weiboContentHasShortUrl:_text]){
                
                [self getShorturlInfo:_text isRetweet:NO];
            }else{
                
                //获取微博话题
                [self getTopicInfo:_text];
            }
            ...
            if(_page_type != 0){
                ...
                NSLog(@"为了找到视频的pageType-%zd", _page_type);
            }
        }
    }
    ...
    //微博图片view的高度
        ...
        CGSize graphicsViewSize = [self calculateGraphicsViewSize:_pic_urls];
        
        self.graphicsViewFrame = CGRectMake(HJMargin, cal.sum, graphicsViewSize.width, graphicsViewSize.height);

        cal.add(graphicsViewSize.height).add(HJMargin);
    }
    
    //转发微博View的高度
    if(_retweeted_status != nil){
        
        CGFloat retweetViewHeight = [self calculateRetweetViewHeight];
        
        //转发微博的frame
        self.retweetViewFrame = CGRectMake(0, cal.sum, ScreenWidth, retweetViewHeight);
        
        cal.add(retweetViewHeight).add(HJMargin);
    }
    
    //分割线高度
    cal.add(1).add(HJMargin);
